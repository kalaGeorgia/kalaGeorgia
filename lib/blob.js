...
async function getGalleryBlobUrl() {
  const { blobs } = await list({ prefix: GALLERY_PATH });
  const match = blobs.find(function (b) { return b.pathname === GALLERY_PATH; });
  return match ? match.url : null;
}

async function readGallery() {
  var empty = { photos: [], videos: [] };
  try {
    const url = await getGalleryBlobUrl();
    if (!url) return empty;
    const res = await fetch(url, { cache: 'no-store' });
    if (!res.ok) return empty;
    const data = await res.json();
...
async function writeGallery(gallery) {
  await put(GALLERY_PATH, JSON.stringify(gallery), {
    access: 'public',
    addRandomSuffix: false,
    allowOverwrite: true,
    contentType: 'application/json'
...
async function getFleetBlobUrl() {
  const { blobs } = await list({ prefix: FLEET_PATH });
  const match = blobs.find(function (b) { return b.pathname === FLEET_PATH; });
  return match ? match.url : null;
}

async function readFleet() {
  try {
    const url = await getFleetBlobUrl();
    if (!url) return null;
    const res = await fetch(url, { cache: 'no-store' });
...
async function writeFleet(fleet) {
  await put(FLEET_PATH, JSON.stringify(fleet), {
    access: 'public',
    addRandomSuffix: false,
    allowOverwrite: true,
    contentType: 'application/json'
...
module.exports = { readContent, writeContent, getContentBlobUrl, CONTENT_PATH, readStats, writeStats, STATS_PATH, readTours, writeTours, TOURS_PATH, readGallery, writeGallery, GALLERY_PATH, readFleet, writeFleet, FLEET_PATH };
